...
BEGIN
    ...
    ----------------------------------------------------------------------------
    -- 1) app_user FAILS calling app.alter_column_type (ER 42501)
    ----------------------------------------------------------------------------
    IF pg_has_role(SESSION_USER, 'app_user', 'member') THEN
        RETURN NEXT throws_like(
            format('SELECT app.alter_column_type(''test.%s'', ''x'', ''int4'');', tname),
            '%alter_column_type not allowed. session_user=%',
            'app_user cannot call app.alter_column_type'
        );
    ELSE
        RETURN NEXT pass('skipped (SESSION_USER is not app_user)');
    END IF;

    ----------------------------------------------------------------------------
    -- Prepare a simple table for db_migrator/table_creator tests
    ----------------------------------------------------------------------------
    EXECUTE format('CREATE TABLE %s (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, x text);', full_name);

    ----------------------------------------------------------------------------
    -- 2) db_migrator PASSES via helper (TEXT -> INT4)
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    -- 3) table_creator PASSES via helper (TEXT -> INT4)
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    -- 4) db_migrator FAILS with raw ALTER TABLE
    ----------------------------------------------------------------------------
    IF pg_has_role(SESSION_USER, 'db_migrator', 'member') THEN
    ...
    ----------------------------------------------------------------------------
    -- 5) table_creator FAILS with raw ALTER TABLE
    ----------------------------------------------------------------------------
    IF pg_has_role(SESSION_USER, 'table_creator', 'member') THEN
        RETURN NEXT throws_ok(
            format('ALTER TABLE %s ALTER COLUMN x TYPE bigint;', full_name),
    ...
    -- Cleanup
    EXECUTE format('DROP TABLE IF EXISTS %s;', full_name);
